...
def test_fake_metadata_generation():
    ...
    try:
        ...
        print(f"   ⚠️  ISSUE: Metadata is fake (not real retrieval)")
        print(f"   ⚠️  Expected: Real document retrieval from vector DB")
        print(f"   ⚠️  Actual: Mock metadata generation")
        print(f"   ⚠️  Impact: RLS enforcement based on fake documents")
        ...
        print(f"   ⚠️  ISSUE: No real document content (just metadata)")
        print(f"   ⚠️  Expected: Real document content for RLS checks")
        print(f"   ⚠️  Actual: Only IDs and metadata")
...
def test_arbitrary_score_formula():
    ...
    try:
        ...
        for i, score in enumerate(scores[:len(expected_scores)]):
            ...
            print(f"   Item {i+1}: score={score:.3f}, expected={expected_scores[i]:.3f}, match={matches[i]}")
        
        if all(matches):
            print(f"   ⚠️  ISSUE: Scores use arbitrary formula: 0.9 - (i * 0.1)")
            print(f"   ⚠️  Expected: Real similarity scores from vector search")
            print(f"   ⚠️  Actual: Hardcoded formula (line 25)")
            print(f"   ⚠️  Impact: Scores don't reflect actual document similarity")
...
def test_no_actual_similarity_search():
    ...
    try:
        ...
        with patch('src.sec_agent.metadata_generator') as mock_module:
            metadata = generate_retrieval_metadata(query_context, user_tenant)
            ...
            print(f"   ⚠️  ISSUE: Function doesn't query vector DB")
            print(f"   ⚠️  Expected: similarity_search() call to vector store")
            print(f"   ⚠️  Actual: No vector DB query (generates mock data)")
            print(f"   ⚠️  Impact: RLS enforcement based on simulated documents")
        ...
        if len(metadata) == len(metadata2):
            print(f"   ⚠️  ISSUE: Same topic produces same metadata count")
            print(f"   ⚠️  No actual similarity search based on query text")
...
def test_topic_based_generation():
    ...
    try:
        ...
        print(f"   ⚠️  ISSUE: Metadata generated per topic, not query similarity")
        print(f"   ⚠️  Expected: Retrieve documents based on query semantic similarity")
        print(f"   ⚠️  Actual: Generate metadata for each topic in query_context")
        print(f"   ⚠️  Impact: Ignores actual query text similarity")
...
def test_hardcoded_cross_tenant_doc():
    ...
    try:
        ...
        cross_tenant_items = [item for item in metadata2 if item["tenant_id"] != query_context2["target_tenant"]]
        print(f"   Cross-tenant items: {len(cross_tenant_items)}")
        
        if cross_tenant_items:
            for item in cross_tenant_items:
                print(f"   Cross-tenant item: tenant_id={item['tenant_id']}, embedding_id={item['embedding_id']}")
                if item["embedding_id"] == "emb-cross-001":
                    print(f"   ⚠️  ISSUE: Hardcoded cross-tenant doc (line 31)")
                    print(f"   ⚠️  Expected: Real cross-tenant documents from DB")
                    print(f"   ⚠️  Actual: Always same cross-tenant doc (emb-cross-001)")
        ...
        print(f"   Query 3: Same tenant with explicit target")
        print(f"   Metadata items: {len(metadata3)}")
        
        cross_tenant_items3 = [item for item in metadata3 if item["tenant_id"] != query_context3["target_tenant"]]
...
def test_no_k_parameter():
    ...
    try:
        ...
        print(f"   ⚠️  ISSUE: No k parameter (retrieve top-k documents)")
        print(f"   ⚠️  Expected: Function accepts k parameter and returns top-k docs")
        print(f"   ⚠️  Actual: Returns metadata per topic in query_context")
        print(f"   ⚠️  Impact: Cannot control how many documents to retrieve")
        ...
        if "k" not in params:
            print(f"   ⚠️  ISSUE: Function doesn't have k parameter")
...
def test_tenant_policy_integration():
    ...
    try:
        ...
        print(f"   ✅ Sensitivities match tenant policies")
...
def test_empty_topics():
    ...
    try:
        ...
        if len(metadata) == 0:
            print(f"   ✅ Returns empty list for no topics")
        else:
...
def test_edge_cases():
    ...
    try:
        ...
        try:
            ...
            print(f"   ✅ Uses default empty list for missing topics")
        except Exception as e:
        ...
        print(f"   Expected: INTERNAL (default)")
        
        assert sensitivity == "INTERNAL", "Should use default sensitivity for invalid tenant"
        print(f"   ✅ Uses default sensitivity for invalid tenant")
