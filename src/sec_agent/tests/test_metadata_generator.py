"""
Unit Tests for generate_retrieval_metadata function

Tests the vector store implementation with proper mocking and benchmarks.
...
# Now load metadata_generator (imports after path setup - needed for test package layout)
metadata_gen_path = project_root / "src" / "sec_agent" / "metadata_generator.py"
metadata_spec = importlib.util.spec_from_file_location(
    "sec_agent.metadata_generator", metadata_gen_path
)
...
metadata_spec.loader.exec_module(metadata_module)  # noqa: E402

generate_retrieval_metadata = metadata_module.generate_retrieval_metadata


class TestGenerateRetrievalMetadata(unittest.TestCase):
    """Unit tests for generate_retrieval_metadata function"""
    ...
    def setUp(self):
        ...
        self.func = generate_retrieval_metadata
    ...
    def test_connects_to_real_vector_store(self):
        ...
        self.mock_vector_store.similarity_search_with_score.return_value = [(self.mock_doc1, 0.85)]

        # Call function
        _ = self.func(query_context, user_tenant, self.mock_vector_store)
        ...
        # Verify query text
        self.assertEqual(call_args[0][0], "security", "Query text should match input")

        # Verify number of results requested
        self.assertEqual(call_args[1]["k"], 5, "Should request 5 results")

        # Verify filter is present
        self.assertIn("filter", call_args[1], "Filter parameter should be present")
    ...
    def test_returns_real_document_ids_and_content(self):
        ...
        for field in required_fields:
            self.assertIn(field, result[0], f"Result should contain '{field}' field")

        # Verify real document IDs (from mock doc metadata)
        self.assertEqual(
            result[0]["document_id"], "doc-001", "Document ID should match stored document"
        )
        self.assertEqual(
            result[0]["embedding_id"], "emb-001", "Embedding ID should match stored document"
        )
        ...
        self.assertGreater(len(result[0]["content"]), 0, "Content should not be empty")
    ...
    def test_handles_errors_gracefully(self):
        ...
        self.mock_vector_store.similarity_search_with_score.side_effect = Exception(
            "Vector store error"
        )
    ...
    def test_real_retrieval_different_queries(self):
        ...
        security_results = self.func({"query": "security"}, "tenant_a", self.mock_vector_store)
        ...
        finance_results = self.func({"query": "finance"}, "tenant_a", self.mock_vector_store)
        ...
        marketing_results = self.func({"query": "marketing"}, "tenant_a", self.mock_vector_store)

        # Verify different queries return different top results
        self.assertEqual(
            security_results[0]["document_id"],
            "doc-security-protocols",
            "Security query should return security document as top result",
        )
        self.assertEqual(
            finance_results[0]["document_id"],
            "doc-budget-report",
            "Finance query should return finance document as top result",
        )
        self.assertEqual(
            marketing_results[0]["document_id"],
            "doc-campaign-strategy",
    ...
    def test_retrieval_scores_realistic(self):
        ...
        results = self.func({"query": "security protocols"}, "tenant_a", self.mock_vector_store)
        ...
        self.assertTrue(
            has_large_gap or score_diffs[1] > 0.2,
        ...
        self.assertEqual(scores[0], max(scores), "Highest similarity score should be first")

        # Verify scores are different (not all the same)
        unique_scores = set(scores)
        self.assertGreater(len(unique_scores), 1, f"Scores should vary, got: {scores}")
    ...
    def test_real_document_ids(self):
        ...
        results = self.func({"query": "test"}, "tenant_a", self.mock_vector_store)
        ...
        for result in results:
            ...
            self.assertNotEqual(doc_id, "doc-empty", "Document ID should not be placeholder")
            self.assertNotEqual(emb_id, "emb-empty", "Embedding ID should not be placeholder")

            # Verify IDs have reasonable length (not too short)
            self.assertGreater(
                len(doc_id), 5, f"Document ID '{doc_id}' should be more than 5 characters"
            )
            self.assertGreater(
                len(emb_id), 5, f"Embedding ID '{emb_id}' should be more than 5 characters"
            )
...
class TestGenerateRetrievalMetadataRealPerformance(unittest.TestCase):
    """Performance benchmarks for generate_retrieval_metadata"""
    ...
    def test_response_time_under_100ms(self):
        ...
        self.assertLess(
            response_time_ms, 100, f"Response time {response_time_ms}ms exceeds 100ms threshold"
        )
        self.assertGreater(len(result), 0)
    ...
    def test_response_time_under_500ms_with_large_result_set(self):
        ...
        self.assertLess(
            response_time_ms, 500, f"Response time {response_time_ms}ms exceeds 500ms threshold"
        )
    ...
    def test_handles_multiple_concurrent_calls(self):
        ...
        self.assertLess(
            avg_time_ms, 50, f"Average response time {avg_time_ms}ms exceeds 50ms threshold"
        )
...
def run_performance_benchmarks():
    ...
    suite = unittest.TestLoader().loadTestsFromTestCase(
        TestGenerateRetrievalMetadataRealPerformance
    )
...
def print_test_summary(unit_result, integration_result, perf_result, integration_test_result=None):
    ...
    print(
        f"   Tests: {unit_result.testsRun}, Failures: {len(unit_result.failures)}, Errors: {len(unit_result.errors)}"
    )
    ...
    if integration_test_result:
        ...
        if skipped_count > 0:
            ...
            print("   Reason: langchain_core not available")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 70)
    print("🧪 UNIT TESTS FOR generate_retrieval_metadata")
    print("   (Using MagicMock - no real embeddings needed)")
    print("=" * 70)

    suite = unittest.TestLoader().loadTestsFromTestCase(TestGenerateRetrievalMetadata)
    runner = unittest.TextTestRunner(verbosity=2)
    ...
    if unit_result.wasSuccessful():
        suite = unittest.TestLoader().loadTestsFromTestCase(
            TestGenerateRetrievalMetadataRealIntegration
        )
        runner = unittest.TextTestRunner(verbosity=2)
        integration_test_result = runner.run(suite)
        integration_result = (
            integration_test_result.wasSuccessful() and len(integration_test_result.skipped) == 0
        )
    else:
    ...
    all_passed = print_test_summary(
        unit_result, integration_result, perf_result, integration_test_result
    )
