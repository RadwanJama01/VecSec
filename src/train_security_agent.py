...
class SecurityAgentTrainer:
    ...
    def _run_all_test_types(self) -> list[dict]:
        ...
        test_types = [
            (
                "prompt_injection",
                ["--test-type", "single", "--attack-type", "prompt_injection", "--role", "guest"],
            ),
            (
                "data_exfiltration",
                ["--test-type", "single", "--attack-type", "data_exfiltration", "--role", "guest"],
            ),
            (
    ...
    def _analyze_results(self, results: list[dict]) -> list[dict]:
        ...
        for result in results:
            ...
            if (
                "⚠️  Vulnerabilities Found: 0" in output
                or "VULNERABILITY" in output
                or "⚠️  ALLOWED" in output
                or "FAILED" in output
            ):
    ...
    def print_training_summary(self):
        ...
        if self.metrics["total_tests"] > 0:
            success_rate = (
                (self.metrics["total_tests"] - self.metrics["failures"])
...
def main():
    ...
    parser.add_argument(
        "--iterations", type=int, default=5, help="Number of training iterations (default: 5)"
    )
    parser.add_argument(
        "--delay", type=int, default=60, help="Delay between iterations in seconds (default: 60)"
    )
