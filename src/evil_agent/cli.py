...
def main():
    ...
    parser.add_argument(
        "--batch", type=int, default=0, help="Generate multiple examples per attack type"
    )
    parser.add_argument("--seed", type=int, default=None, help="Optional random seed")
    parser.add_argument(
        "--use-llm", action="store_true", help="Use LLM APIs to generate sophisticated attacks"
    )
    parser.add_argument(
        "--llm-provider",
        default="google",
        choices=["google", "openai", "flash"],
        help="LLM provider to use",
    )
    parser.add_argument(
        "--test-agent", action="store_true", help="Test generated attacks against Sec_Agent.py"
    )
    parser.add_argument(
        "--privilege-escalation",
        action="store_true",
        help="Generate privilege escalation attacks with mismatched roles",
    )
    ...
    if args.test_agent:
        ...
        if attack_data:
            test_attack_against_agent(
                attack_data["example"]["query"], args.tenant_id, args.clearance
            )
