...
def test_basic_attack_generation():
    ...
    try:
        ...
        print("   Attack type: prompt_injection")
        ...
        print("   ✅ Basic attack generation works")
...
def test_static_attack_generation():
    ...
    try:
        ...
        for attack_type in ["prompt_injection", "data_exfiltration", "jailbreak"]:
            ...
            assert query in catalog_attacks, f"Query should be from {attack_type} catalog"
            assert metadata["generation_method"] == "static", "Should use static generation method"

        print("   ✅ Static attack generation works correctly")
...
def test_llm_attack_generation():
    ...
    try:
        ...
        if "API key not configured" in query or "Error calling" in query:
            print("   ⚠️  ISSUE: LLM generation fails silently and falls back")
            print("   ⚠️  Expected: Raise error or return None when LLM fails")
            print("   ⚠️  Actual: Returns error message as query string")
        elif metadata["generation_method"] == "static":
            print("   ⚠️  ISSUE: LLM disabled but still marked as 'static'")
            print("   ⚠️  Expected: Should use LLM or raise error")
        ...
        with patch("src.evil_agent.attack_generator.get_llm_client") as mock_get_client:
            ...
            print("   With mocked LLM:")
            print(f"     Query: {query2}")
            print(f"     Generation method: {metadata2.get('generation_method')}")

            if query2 == "Generated attack prompt":
                print("   ✅ LLM generation works with mocked client")
            else:
                print("   ⚠️  LLM generation may not be working correctly")
...
def test_random_attack_type_selection():
    ...
    try:
        ...
        print("   Attack type not specified")
        print(f"   Selected type: {attack_type}")

        assert attack_type in ATTACK_TYPES, f"Selected attack type should be valid: {attack_type}"
        ...
        assert attack_type2 in ATTACK_TYPES, f"Should select random valid type: {attack_type2}"

        print("   ✅ Random attack type selection works")
...
def test_seed_reproducibility():
    ...
    try:
        ...
        result1 = generate_attack(
            user_id="test_user", tenant_id="tenantA", clearance="INTERNAL", seed=42, use_llm=False
        )
        ...
        if query1 == query2:
            print("   ✅ Seed produces reproducible queries")
        else:
            print("   ⚠️  Seed does not produce reproducible queries")
            print("   ⚠️  Expected: Same seed = same query")
            print("   ⚠️  Actual: Same seed = different query")
...
def test_metadata_structure():
    ...
    try:
        ...
        try:
            ...
            print("   ✅ attack_id is valid UUID")
        except ValueError:
            print("   ⚠️  ISSUE: attack_id is not valid UUID")
        ...
        try:
            ...
            print("   ⚠️  ISSUE: timestamp is not valid ISO format")
...
def test_edge_cases():
    ...
    try:
        ...
        result = generate_attack(user_id="", tenant_id="", clearance="", use_llm=False)
        ...
        try:
            result = generate_attack(
                user_id=None, tenant_id="tenantA", clearance="INTERNAL", use_llm=False
            )
        ...
        long_string = "x" * 1000
        result = generate_attack(
            user_id=long_string, tenant_id="tenantA", clearance="INTERNAL", use_llm=False
        )

        assert result["example"]["user_id"] == long_string, "Should accept long user_id"
        ...
        assert result["example"]["role"] == "invalid_role", "Should accept any role string"
