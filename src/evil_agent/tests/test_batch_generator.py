...
def test_basic_batch_generation():
    ...
    try:
        ...
        print("   ✅ Basic batch generation works")
...
def test_batch_size():
    ...
    try:
        ...
        print("   ✅ Batch size works correctly")
...
def test_attack_type_selection():
    ...
    try:
        ...
        print("   ✅ Attack type selection works correctly")
...
def test_llm_batch_generation():
    ...
    try:
        ...
        if "llm" in methods:
            ...
            print("   ⚠️  LLM generation fell back to static")
            print("   ⚠️  Expected: Use LLM or raise error")
            print("   ⚠️  Actual: Falls back to static silently")
    ...
    try:
        ...
        for role in roles:
            ...
            for result in results:
                ...
                assert example_role == role, f"Role should be {role}, got {example_role}"

            print(f"   Role {role}: ✅ All attacks have correct role")

        print("   ✅ Role parameter works correctly")
...
def test_metadata_consistency():
    ...
    try:
        ...
        assert len(set(user_ids)) == 1, "All attacks should have same user_id"
        ...
        assert len(set(tenant_ids)) == 1, "All attacks should have same tenant_id"
        ...
        assert len(set(clearances)) == 1, "All attacks should have same clearance"
...
def test_empty_batch():
    ...
    try:
        ...
        assert isinstance(results, list), "Should return a list"
        assert len(results) == 0, "Should return empty list for empty attack types"
...
def test_invalid_attack_types():
    ...
    try:
        ...
        try:
            ...
            if len(results) == 0:
                print("   ✅ Returns empty list for invalid types")
            else:
                print("   ⚠️  ISSUE: Generates attacks even with invalid types")
                print("   ⚠️  Expected: Raise error or return empty list")
                print("   ⚠️  Actual: May generate attacks with default/random types")
...
def test_edge_cases():
    ...
    try:
        ...
        try:
            ...
            print("   ⚠️  ISSUE: Should not allow negative count")
        except Exception as e:
        ...
        try:
            results = generate_batch(
                user_id=None, tenant_id=None, clearance=None, count_per_type=1, use_llm=False
            )
            print(f"   None values: {len(results)} attacks")
            print("   ⚠️  Should handle None values or raise error")
        except Exception as e:
