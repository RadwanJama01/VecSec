...
def test_attack_types():
    ...
    try:
        ...
        print("   ✅ get_attack_types() works correctly")
...
def test_hardcoded_attacks():
    ...
    try:
        ...
        print("   ⚠️  ISSUE: Attacks are hardcoded (not configurable)")
        print("   ⚠️  Expected: Configurable via file/DB/API")
        print("   ⚠️  Actual: Hardcoded in Python code")
        print("   ⚠️  Impact: Cannot add new attacks without code changes")
...
def test_get_attack_type():
    ...
    try:
        from src.evil_agent.attack_catalog import ATTACK_TYPES, get_attack_type
        ...
        print("   Attack type: data_exfiltration")
        ...
        print("   Attack type: invalid_type")
        print(f"   Attacks: {invalid}")

        assert invalid == [], "Should return empty list for invalid type"
        print("   ✅ Get attack type works correctly")
        print("   ⚠️  ISSUE: Returns empty list for unknown types (no error)")
        print("   ⚠️  Expected: Raise ValueError or return None")
        print("   ⚠️  Actual: Returns empty list (line 61)")
...
def test_list_attack_types():
    ...
    try:
        from src.evil_agent.attack_catalog import ATTACK_TYPES, list_attack_types
        ...
        assert len(attack_types_list) == len(ATTACK_TYPES), "Should return all attack types"
        ...
        for expected_type in expected_types:
            assert expected_type in attack_types_list, f"Should include {expected_type}"

        print("   ✅ List attack types works correctly")
        ...
        for attack_type, attacks in ATTACK_TYPES.items():
            ...
            if len(attacks) > 1:
                ...
                if unique_attacks < len(attacks):
                    print("     ⚠️  ISSUE: Duplicate attacks found")
                elif unique_attacks == 1:
                    print("     ⚠️  ISSUE: Only one unique attack (needs variety)")

            # Show example
            if len(attacks) > 0:
                print(f"     Example: {attacks[0][:80]}...")

        print("   ✅ Attack content quality checked")
...
def test_attack_type_coverage():
    ...
    try:
        ...
        if low_coverage:
            print("   ⚠️  ISSUE: Some attack types have low coverage:")
            for attack_type, count in low_coverage:
                print(f"     - {attack_type}: {count} attacks")
        else:
            print("   ✅ All attack types have sufficient coverage")
...
def test_edge_cases():
    ...
    try:
        from src.evil_agent.attack_catalog import get_attack_type
...
def test_attack_catalog_immutability():
    ...
    try:
        ...
        if len(ATTACK_TYPES) > original_count:
            print("   ⚠️  ISSUE: Catalog can be modified directly")
            print("   ⚠️  Expected: Immutable catalog or protected access")
            print("   ⚠️  Actual: Catalog is mutable dict")
            print("   ⚠️  Impact: Attacks can be changed at runtime")

            # Clean up
            del ATTACK_TYPES["test_type"]
        else:
            print("   ✅ Catalog is protected from modification")

        # Check if there's a way to protect catalog
        print("   ⚠️  ISSUE: No catalog protection mechanism")
        print("   ⚠️  Expected: Read-only access or validation before changes")
        print("   ⚠️  Actual: Catalog is regular dict (mutable)")
