...
def test_basic_privilege_escalation():
    ...
    try:
        ...
        print(f"   ✅ Basic privilege escalation works")
...
def test_hardcoded_escalation_scenarios():
    ...
    try:
        ...
        print(f"   ⚠️  ISSUE: Escalation scenarios are hardcoded (only {len(unique_scenarios)} scenarios)")
        print(f"   ⚠️  Expected: Algorithmic generation of ALL role/clearance/tenant combinations")
        print(f"   ⚠️  Actual: Hardcoded list with limited scenarios (line 39-46)")
        print(f"   ⚠️  Impact: Cannot test all possible privilege escalation paths")
...
def test_scenario_types():
    ...
    try:
        ...
        if target_clearance_scenarios:
            ...
            print(f"   Clearance escalation patterns:")
        ...
        if target_tenant_scenarios:
            ...
            print(f"   Tenant escalation patterns:")
            for pattern, count in tenant_types.items():
                print(f"     {pattern}: {count}")
        
        print(f"   ✅ Scenario types identified")
...
def test_query_generation():
    ...
    try:
        ...
        if len(unique_queries) < len(queries_seen) * 0.5:
            print(f"   ⚠️  ISSUE: Low query variety (repeated queries)")
            print(f"   ⚠️  Expected: More variety in generated queries")
...
def test_attack_type_parameter():
    ...
    try:
        ...
        for attack_type in attack_types:
            ...
            print(f"   ✅ Attack type parameter works")
...
def test_seed_reproducibility():
    ...
    try:
        ...
        if scenario1 == scenario2:
            print(f"   ✅ Seed produces reproducible scenarios")
        else:
            print(f"   ⚠️  Seed does not produce reproducible scenarios")
            print(f"   ⚠️  Expected: Same seed = same scenario")
            print(f"   ⚠️  Actual: Same seed = different scenario")
...
def test_missing_combinations():
    ...
    try:
        ...
        # Expected all combinations
        roles = ["admin", "superuser", "analyst", "guest"]
        clearances = ["PUBLIC", "INTERNAL", "CONFIDENTIAL", "SECRET"]
        target_clearances = ["PUBLIC", "INTERNAL", "CONFIDENTIAL", "SECRET"]
        ...
        print(f"   ⚠️  Expected: All possible role/clearance/tenant combinations")
        print(f"   ⚠️  Actual: Hardcoded list with 6 scenarios")
        print(f"   ⚠️  Missing combinations:")
        print(f"     - Superuser escalation attempts")
        print(f"     - Admin escalation attempts")
        print(f"     - All clearance level combinations")
        print(f"     - All tenant combinations")
